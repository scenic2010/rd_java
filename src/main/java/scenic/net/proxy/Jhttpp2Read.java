...
public class Jhttpp2Read extends Thread {
	...
	private HTTPSession connection;
	private static HttpServer server;

	public Jhttpp2Read(HttpServer server, HTTPSession connection,BufferedInputStream l_in, BufferedOutputStream l_out) {
		this.in = l_in;
		this.out = l_out;
	...
	private void read() {
		...
		try {
			if (connection.getStatus() != HTTPSession.SC_CONNECTING_TO_HOST) // *uaaahhh*:fixes a very strange bug
