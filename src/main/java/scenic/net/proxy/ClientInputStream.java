...
public class ClientInputStream extends BufferedInputStream {
	...
	public ClientInputStream(HttpServer server, HTTPSession connection,
			InputStream a) {
		super(a);
	...
	public int read(byte[] bu) throws IOException {
		...
		if (header_length == 0) {
			...
			statuscode = HTTPSession.SC_CONNECTION_CLOSED;
		}
		...
		boolean temp1 = (statuscode == HTTPSession.SC_OK);
...
//		return header_length ;
	...
	private boolean forReadTheHeadOfHttp() {
		...
		case -1:
			//不支持的请求方法，可以暂时不用考虑
			statuscode = HTTPSession.SC_NOT_SUPPORTED;
			return false;
		case 2:
			//用CONNECT方法请求
			isSSL = true;
		default: //GET POST 请求
			...
			if (!address.equals(remote_host)) {
				...
				statuscode = HTTPSession.SC_CONNECTING_TO_HOST;
				remote_host = address;
			}
