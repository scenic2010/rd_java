...
public class Jhttpp2Read extends Thread {
	...
	private scenic.study.net.proxy.HTTPSession connection;
	private static HttpServer server;

	public Jhttpp2Read(HttpServer server, scenic.study.net.proxy.HTTPSession connection, BufferedInputStream l_in, BufferedOutputStream l_out) {
		this.in = l_in;
		this.out = l_out;
	...
	private void read() {
		...
		try {
			if (connection.getStatus() != scenic.study.net.proxy.HTTPSession.SC_CONNECTING_TO_HOST) // *uaaahhh*:fixes a very strange bug
				connection.getLocalSocket().close();
			// why? If we are connecting to a new host (and this thread is already running!) , 
			//the upstream socket will be closed. So we get here and close our owndownstream socket..... and the browser
