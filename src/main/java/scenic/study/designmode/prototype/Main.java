...
import scenic.study.designmode.prototype.addNewTrash2.Cardboard;
import scenic.study.designmode.prototype.solution5.DynamicTrash;
import scenic.study.designmode.prototype.solution3.Fillable;
import scenic.study.designmode.prototype.solution3.VectorFillable;
...
//        VectorFillable 在这里是一个Adapter 设计
        fillAllTrash_for_solution3(types, new VectorFillable(allTrashes));
    ...
    public void solution5() {
        String[] types = readTypesFromSource();
        DynamicTrash allTrashes = new DynamicTrash();
//        VectorFillable 在这里是一个Adapter 设计
    ...
    private void fillAllTrash_for_solution2(String[] types, TrashVector allTrashes) {
        for (int i = 0; i < 100; i++) {
            ...
            double weight = Math.random() * 1000;
            allTrashes.add(Trash.facotry(new Trash.Info().setName(typeStr).setWeight(weight)));
        }
    }
     ...
     * @param fillable
     */
    public static void fillAllTrash_for_solution3(String[] types, Fillable fillable) {
        for (int i = 0; i < 100; i++) {
            int typeIndex = random.nextInt(types.length);

            String typeStr = types[typeIndex];
            double weight = Math.random() * 1000;
            fillable.addTrash(Trash.facotry(new Trash.Info().setName(typeStr).setWeight(weight)));
        }
    }
}
